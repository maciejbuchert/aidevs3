...
const prompts: AgentPrompts = {
    ...
    describe: {
        systemPrompt: async (state: IState) => {
...
const tools: Record<string, Tool> = {
    get_notes: {
        instruction: `No payload required. Response format: Text notes.`,
        description: `Use this action to fetch text notes.`,
        function: async () => {
            try {
                const response = await got.get('https://centrala.ag3nts.org/dane/barbara.txt');
                console.log(await response.body);
                return await response.body;
            } catch (error) {
                console.error('Error fetching URL:', error);
                return 'Failed to fetch notes, please try again.';
            }
        }
    },
    query_to_api: {
        instruction: `Required payload: {"type": "'people' or 'places'", "query": "city name or human name. Provide value without diacritical characters. For human names, use only firstname"}. Response format: The response from API is in JSON.`,
        description: `Use this action to query the API to retrieve information about some people or places.`,
        function: async ({ type, query }: { type: string; query: string }) => {
            try {
                return await got.post(`https://centrala.ag3nts.org/${type}`, {
                    json: {
                ...
                console.error('Error fetching URL:', error);
                return 'Failed to fetch, please try again.';
            }
        },
    },
    final_answer: {
        instruction: `Required payload: {"answer": "Your answer"}. Response format: Text.`,
        description: `Use this action to provide the final answer.`,
        function: async ({ answer }: { answer: string }) => {
            finalAnswer = answer;
            return 'Answer saved.';
        }
    }
...
const message = new HumanMessage('You have to help us track down Barbara Zawadzka. We need to know where she is right now. Can you help us with that?. Use notes as a basic information. Please, use tools to get the information.');
...
try {
    ...
    const response = await aiDevs.sendTask('loop', finalAnswer);
    if(response) {
