...
const tools: Record<string, Tool> = {
    get_file_contents: {
        instruction: `Required payload: {"url": "URL that needs to be downloaded"} 'url' property is required.`,
        description: `Use this action to fetch file content of the URL that was explicitly mentioned by the user. You're strictly forbidden to pass any other URLs including not mentioned domains and/or paths because you generate additional costs that will be wasted.`,
    ...
    send_request: {
        instruction: `Required payload: {"url": "URL with form", body: "JSON object that will be send to url",}. Response format: JSON Object`,
        description: `Use this action make HTTPS requests`,
        function: async ({ url, body }: { url: string; body: object | string }) => {
...
const agent = new Agent(prompts, tools, 'gpt-4o');

const message = new HumanMessage('Fetch file from this url https://xyz.ag3nts.org/files/0_13_4b.txt and analyze. You can use https://xyz.ag3nts.org/verify endpoint to make HTTPS requests. After few requests, response should looks like "{{FLG:PIZZA}}". I need this response');
